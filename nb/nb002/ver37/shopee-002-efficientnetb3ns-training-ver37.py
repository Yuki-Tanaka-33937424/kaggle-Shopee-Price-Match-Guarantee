...
class CFG:
    ...
    size = 512
    scheduler = 'CosineAnnealingLR'  # ['custom', 'ReduceLROnPlateau', 'CosineAnnealingLR', 'CosineAnnealingWarmRestarts']
    batch_size = 24
    epochs = 2  # changed
...
from albumentations import (
    ...
    IAAAdditiveGaussianNoise, Transpose, HueSaturationValue, RandomBrightnessContrast, CoarseDropout, ToSepia
    )
...
#             optimizer.zero_grad()
            global_step += 1
            y_preds_ = model(images, labels)
            loss_train(y_preds_, labels).backward()
...
def train_loop(folds, fold):
    ...
    # model & optimizer
    # ====================================================
    model = CustomEfficientNet(n_classes=CFG.target_size, model_name=CFG.model_name, pretrained=False).to(device)
    model_path = f'../ver36/tf_efficientnet_b3_ns_fold{fold}_best.pth'
    ...
    if CFG.scheduler == 'custom':
        ...
        base_optimizer = Adam
        optimizer = SAM(model.parameters(), base_optimizer, lr=CFG.lr, weight_decay=CFG.weight_decay, amsgrad=False)
