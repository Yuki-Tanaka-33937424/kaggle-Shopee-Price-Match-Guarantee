...
class CustomEfficientNet(nn.Module):
    def __init__(
        ...
        self.use_fc = use_fc
        self.local_fe = CBAM(in_features)
        
        if use_fc:
            self.dropout = nn.Dropout(p=0.5)
            self.classifier = nn.Linear(in_features + in_features, fc_dim)
    ...
    def extract_features(self, x):
        ...
        global_features = self.pooling(features).view(batch_size, -1)
        
        local_features = self.local_fe(features)
        local_features = torch.sum(local_features, dim=[2, 3])
        
        all_features = torch.cat([global_features, local_features], dim=1)
        
        if self.use_fc:
            x = self.dropout(all_features)
            x = self.classifier(x)
            x = self.bn(x)
        return x
